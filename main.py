import sqlite3
import sys
from PyQt6.QtWidgets import (QWidget, QApplication, QTableWidgetItem, QMessageBox, QListWidgetItem)
from PyQt6.QtCore import Qt
from movies import Ui_Form


DATABASE_NAME = '../db/movies_db.db'
...
class Window(QWidget, Ui_Form):
    ...
    def showDetails(self):
        row = self.tableWidget.currentItem().data()
        ...
        result = self.con.execute('''
            SELECT title
            FROM genres
            JOIN movies_genres ON genre_id = id
            WHERE movie_id = ?
        ''', (movie_id,))
